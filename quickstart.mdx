...
<Steps>
  ...
  <Step title="Wait for the Bootstrap Script to Complete">
    ...
    curl -sSf https://temporal.download/cli.sh | sh  # Installs Temporal CLI to ~/.temporalio/bin/
    cp .env.sample .env              # Creates your .env file from the template
    python scripts/check_env.py      # Validates the environment
    ```
