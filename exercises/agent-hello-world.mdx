---
title: "Build Your First OpenAI Agent with Tool Calling"
sidebarTitle: "Agent Hello World"
description: "Create a weather-alert AI agent using OpenAI Agents SDK's @function_tool decorator, learn how LLMs decide when to call tools, and query the National Weather Service API."
---
...
<CardGroup cols={2}>
  <Card title="Tool Calling Basics" icon="wrench">
    Decorate any async function with `@function_tool` to expose it to the LLM as a callable action.
  </Card>
  <Card title="Agent → Tool → Response Flow" icon="arrow-right-arrow-left">
...
<Steps>
  ...
  <Step title="Create the Agent">
    Pass the tool in the `tools` list. The `instructions` field shapes the model's behavior and persona throughout the conversation.
